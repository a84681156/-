...
<head>
    ...
    <!-- 로컬css -->
    <link rel="stylesheet" href="<c:url value='/member/css/crews.css'/>" >
...
<body>
    ...
    <div id="crewsContainer">
    
        <div id="crewsIntro">
            <img src="<c:url value='/img/boardmain.png'/>"  >
            <div>
                CREWS <br>
                개인이 프로젝트 크루를 모집할 수 있는 크루 게시판
            </div>
        </div><!-- crewsIntro -->
            ...
            <div class="container">
                <table class="board-table">
                    <thead>
                     ...
                     <c:forEach var="crewsboardDTO" items="${list}">
	                    <tbody>
							<tr>
								<td class="crewsPostNo">
									${crewsboardDTO.crew_post_no}
								</td>
								<td class="crewsImg">
									<div class="crews_img">
								...
								<td class="crewsTitle">
									<a href="<c:url value= '/crews/read${ph.sc.getQueryString() }&crew_post_no=${crewsboardDTO.crew_post_no }' />">
										${crewsboardDTO.crew_title} 
									</a>
								</td>
								<td class="crewsInfo">
	                                <ul>
	                                    <li>진행지역 : ${crewsboardDTO.crew_local}</li>
	                                    <li>업계경력 : ${crewsboardDTO.crew_career}</li>
	                                    <li>진행기간 : ${crewsboardDTO.crew_time}</li>
	                                </ul>
	                            </td>
	                            <c:choose>
                          		...
                          		<td class="crewsProgress">
                          		 	${crewsboardDTO.crew_progress}
                          		</td>
								<td class="crewsWriter">
									${crewsboardDTO.crew_writer}
								</td>
								<td class="crewsHits">
									${crewsboardDTO.crew_cnt}
								</td>
	                           	<td class="crewsRecomms">
	                           		${crewsboardDTO.crew_recommend}
	                           	</td>
							</tr>
	                    </tbody>
                    </c:forEach>
        ...
        <div class="writeBtn">
            <button type="button">
                <a href="<c:url value='/crews/write'/>">글쓰기</a>
            </button>
        </div>

        <form action="<c:url value='/crews/list'/>" class="srcForm" method="get">
			...
			<div class="pageBtn">
				<ul>
					<c:if test="${ph.showPrev}">
						<li>
		       				<a href="<c:url value='/crews/list${ph.sc.getQueryString(ph.beginPage-1) }'/>"> &lt;</a>   
						</li>
					</c:if>
					<c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
						<li>
		       				<a href="<c:url value='/crews/list${ph.sc.getQueryString(i) }'/>"> ${i}</a>
						</li>
					</c:forEach>
					<c:if test="${ph.showNext}">
						<li>
		       				<a href="<c:url value='/crews/list${ph.sc.getQueryString(ph.endPage+1) }'/>"> &gt;</a>   
						</li>
					</c:if>
   				</ul>
			</div><!-- pageBtn -->
