...
<head>
    ...
    <!-- 로컬css -->
    <link rel="stylesheet" href="<c:url value='/member/css/news.css'/>" >
        ...
        <div id="board-list">
            <div class="container">
                <table class="board-table">
                    ...
                    <c:forEach var="newsboardDTO" items="${list}">
	                    <tbody>
							<tr>
								<td class="newsPostNo">
									${newsboardDTO.news_post_no}
								</td>
								<td class="newsImg">
									<div class="news_img">
								...
								<td class="">
									<a href="<c:url value= '/news/read${ph.sc.getQueryString() }&news_post_no=${newsboardDTO.news_post_no }' />">
										${newsboardDTO.news_title} 
									</a>
								</td>
	                            <c:choose>
								...
								<td class = "newsWriter">
									${newsboardDTO.news_writer}
								</td>
								<td class="newsHits">
									${newsboardDTO.news_cnt}
									
								</td>
	                           	<td class="newsRecomms">
	                           		${newsboardDTO.news_recommend}
	                           	</td>
							</tr>
	                    </tbody>
                    </c:forEach>
       	...
       	<form action="<c:url value='/news/list'/>" class="srcForm" method="get">
			...
			<div class="pageBtn">
				<ul>
					<c:if test="${ph.showPrev}">
						<li>
		       				<a href="<c:url value='/news/list${ph.sc.getQueryString(ph.beginPage-1) }'/>"> &lt;</a>   
						</li>
					</c:if>
					<c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
						<li>
		       				<a href="<c:url value='/news/list${ph.sc.getQueryString(i) }'/>"> ${i}</a>
						</li>
					</c:forEach>
					<c:if test="${ph.showNext}">
						<li>
		       				<a href="<c:url value='/news/list${ph.sc.getQueryString(ph.endPage+1) }'/>"> &gt;</a>   
						</li>
					</c:if>
   				</ul>
			</div><!-- pageBtn -->
